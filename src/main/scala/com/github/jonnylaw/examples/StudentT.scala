...
trait TModel {
  val tparams = Parameters.leafParameter(
    Some(0.3),
    SdeParameter.ornsteinParameter(DenseVector(0.0), DenseMatrix((3.0)), DenseVector(3.0), DenseVector(1.0), DenseVector(0.5)))
  val seasParams: Parameters = Parameters.leafParameter(
    None,
    SdeParameter.ornsteinParameter(
      DenseVector.fill(6)(0.0), 
      diag(DenseVector.fill(6)(3.0)),
      DenseVector.fill(6)(2.0), DenseVector.fill(6)(0.5), DenseVector.fill(6)(0.3)))
...
object GetSeasTParams extends App with TModel {
  ...
  val mll = ParticleFilter.likelihood(ParticleFilter.multinomialResampling, data, 200) compose unparamMod
