...
object DetermineSeasonalParameters extends App with SeasonalTestModel with DataProtocols {
  ...
  def iters(covariance: DenseMatrix[Double], initP: Parameters, chain: Int): Future[IOResult] = for {
    ...
    filter = (p: Parameters) => ParticleFilter.likelihood(data.toVector, resample, 500)(mod(p))
    pmmh = ParticleMetropolisSerial(filter, initP, Parameters.perturbMvn(cholesky(covariance)), prior)
    io <- pmmh.
      params.
      take(10000).
